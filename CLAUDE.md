...
The cron route additionally calls `createDepoSnapshot({ comment: "Автоснимок" })` after the balance check, so `/snapshots` gets one frozen copy of депо per day (unlike the TRX snapshot it is not deduped by date — every run appends a row).
